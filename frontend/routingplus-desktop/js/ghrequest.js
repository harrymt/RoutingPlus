...
GHRequest.prototype.createPath = function(url) {
    ...
    // HARRY ADDED
    // Added these to pass in data from the UI to the java program
    if (this.trafficSignals) {
        if ($("#trafficSignals").val() < 2) {
            url += "&trafficSignals=" + $("#trafficSignals").val() - 1;
        } else {
            url += "&trafficSignals=" + $("#trafficSignals").val() * 2;
        }
    }
    if (this.rightTurns) {
        url += "&rightTurns=0";
    ...
    if (this.motorway) {
        if ($("#motorway").val() < 2) {
            url += "&motorway=" + $("#motorway").val() - 1;
        } else {
            url += "&motorway=" + $("#motorway").val() * 2;
        }
    }
    if (this.mainRoads) {
        if ($("#mainRoads").val() < 2) {
            url += "&mainRoads=" + $("#mainRoads").val() - 1;
        } else {
            url += "&mainRoads=" + $("#mainRoads").val() * 2;
        }
    }
    if (this.residential) {
        if ($("#residential").val() < 2) {
            url += "&residential=" + $("#residential").val() - 1;
        } else {
            url += "&mainRoads=" + $("#mainRoads").val() * 2;
        }
    }
