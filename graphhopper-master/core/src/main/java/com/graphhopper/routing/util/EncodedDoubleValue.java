...
public class EncodedDoubleValue extends EncodedValue
{
    ...
    public double getDoubleValue( long flags )
    {
        ...
        double speed = 0;
        if (flags >= 1000)
		// if the highway value has been encoded, return the speed part of the flags
            speed = Double.parseDouble(String.valueOf(flags).substring(2));
        else // else return the decoded flags
            speed = flags;
        return speed;
        //END G52GRP GROUP
    }
