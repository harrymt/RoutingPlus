...
public abstract class AbstractFlagEncoder implements FlagEncoder, TurnCostEncoder
{
    ...
    public long setSpeed( long flags, double speed )
    {
        ...
        speed = Double.parseDouble("12" + String.valueOf(speed));
    ...
    protected static double parseSpeed( String str )
    {
        ...
        try
        {
            ...
            // BEGIN G52GRP GROUP
            // covert speeds with mph as its unit to kph
            int mphInteger = str.indexOf("mph"); // get value
            if (mphInteger > 0)
            {
                str = str.substring(0, mphInteger).trim();
                int result = (int) Math.round(Integer.parseInt(str) * 1.6);
                return result;
            }
            // END G52GRP GROUP
