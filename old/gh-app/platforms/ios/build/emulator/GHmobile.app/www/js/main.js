...
var host = "http://10.154.159.252:8989";
if (host == null) {
    if (location.port === '') {
...
function initMap() {
    ...
    // default
    map = L.map('map', {
        layers: [osm], // Default layer OSM
        zoomControl: false, // Disable view controls
        center: [20.9, -1.1], // Centers the view
        zoom: 10, // Sets the initial zoom level
        minZoom: 2 // Stops you from viewing out to much
    });
    ...
    // var locationMarkerTemp;
    // findLocation(locationMarkerTemp);
    ...
    //         var container = L.DomUtil.create('a', 'location-btn');
    //         L.DomEvent
...
//         watch: false
//     });
//     // error, it creates multiple markers in same place
//     function onLocationFound(e) {
//         locationMarker = new L.Marker(e.latlng);
//         map.addLayer(locationMarker);
//         locationMarker.bindPopup("<b>You</b>").openPopup();

//         $(".location-btn").css("color", "#26bf66");
...
function routeLatLng(request, doQuery) {
    ...
    if (!from || !to) {
        descriptionDiv.html('<small style="color:red; padding-top: 20px; font-size: 15px;">' + tr('locationsNotFound') + '</small>');
        return;
    }
    ...
    request.doRequest(urlForAPI, function (json) {
        ...
        var toggly = $("<button class='btn btn-toggle btn-toggle-moreroute' style='font-size:14px; float: right; font-weight: bold; padding: 0px'>+</button>");
